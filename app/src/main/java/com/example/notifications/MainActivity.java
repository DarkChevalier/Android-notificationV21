...
//				.setNumber(number)//显示数量
	...
	/** 显示大视图风格通知栏 */
	//通知栏默认高度是64dp
	//要使用更高的高度显示多行文字参见老外的详细教程：http://stackoverflow.com/questions/21237495/create-custom-big-notifications
	public void showBigStyleNotify() {
		...
		mBuilder.setStyle(inboxStyle)//设置风格
				.setTicker("测试通知来啦");// 通知首次出现在通知栏，带上升动画效果的
		mNotificationManager.notify(notifyId, mBuilder.build());
	}
...
//		notification.flags = Notification.FLAG_AUTO_CANCEL; //在通知栏上点击此通知后自动清除此通知
		...
		//注意：这里的这个APK是放在assets文件夹下，获取路径不能直接读取的，要通过Copy出去在读或者直接读取自己本地的PATH，这边只是做一个跳转APK，实际打不开的
